...
const SchemaItemPedido = z.object({
  produto_id: textoObrigatorio("Todo item precisa de produto_id."),
  quantidade: z.number().int().positive("Quantidade de cada item deve ser inteiro maior que zero."),
  preco_unitario: z.number().nonnegative("Preço unitário inválido."),
});

const SchemaCriarPedido = z.object({
  fornecedor_id: textoObrigatorio("Informe fornecedor_id."),
  forma_pagamento: z
    .enum(FORMA_PAGAMENTO_COMPRA_LISTA, {
      error: `forma_pagamento inválida. Use: ${FORMA_PAGAMENTO_COMPRA_LISTA.join(", ")}.`,
    })
    .optional()
    .default(FORMA_PAGAMENTO_COMPRA.BOLETO),
  frete: z.number().nonnegative("frete inválido.").optional().default(0),
  desconto: z.number().nonnegative("desconto inválido.").optional().default(0),
  itens: z.array(SchemaItemPedido).min(1, "Informe ao menos um item no pedido."),
});

const SchemaCancelarPedido = z.object({
  motivo: textoObrigatorio("motivo do cancelamento é obrigatório."),
});

const SchemaItemRecebimento = z.object({
  item_pedido_id: textoObrigatorio("Todo item de conferência precisa de item_pedido_id."),
  quantidade_recebida: z.number().int().nonnegative("Quantidade recebida inválida."),
  numero_lote: textoOpcional(),
  data_validade: textoOpcional(),
});

const SchemaReceberPedido = z.object({
  itens: z.array(SchemaItemRecebimento).min(1, "Informe a conferência de cada item recebido."),
  entregue_em: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "entregue_em deve estar no formato AAAA-MM-DD.")
    .optional()
    .nullable()
...
export async function registrarRotas(app) {
  ...
  app.post(
    "/pedidos",
    { preHandler: [auth.exigirPermissao(PERMISSAO), validarCorpo(SchemaCriarPedido)] },
    async (requisicao, resposta) => {
      ...
      try {
        ...
        const fornecedor = fornecedores.find((registro) => registro.id === fornecedorId);
  ...
  app.post(
    "/pedidos/:id/cancelar",
    { preHandler: [auth.exigirPermissao(PERMISSAO), validarCorpo(SchemaCancelarPedido)] },
    async (requisicao, resposta) => {
      const cancelado = await marcarCancelado({
        pedidoId: requisicao.params.id,
        motivo: requisicao.body.motivo,
      });
  ...
  app.post(
    ...
    async (requisicao, resposta) => {
      ...
      // Regras cruzadas (dependem do pedido no banco, não só da forma do corpo):
      // o item de conferência tem de ser de fato deste pedido, e só exige lote e
      // validade quando alguma quantidade foi recebida.
      for (const item of itens) {
        const original = itensPedido.find((registro) => registro.id === item.item_pedido_id);
        if (!original) {
          return invalido(resposta, "Item de conferência não pertence a este pedido.");
        }

        if (item.quantidade_recebida > 0 && (!item.numero_lote || !item.data_validade)) {
          return invalido(
            resposta,
        ...
        conferidos.push({
          ...
          divergencia: item.quantidade_recebida - original.quantidade,
